...
from app.schemas import UserCreate, UserResponse, OTPVerify, OTPGenerate, Token, UserLogin
...
@router.post("/generate-login-otp")
async def generate_login_otp(
    ...
    otp = generate_otp()
    print(f"OTP generated for {otp_in.email}: {otp}")  # Fallback for testing if SMTP fails
    expires = datetime.now(timezone.utc) + timedelta(minutes=10)
    
    # Store OTP
    otp_record = OTPRecord(email=user.email, otp_code=otp, expires_at=expires)
    db.add(otp_record)
    db.commit()
...
def verify_login_otp(otp_in: OTPVerify, db: Session = Depends(get_db)):
    otp_record = db.query(OTPRecord).filter(
        OTPRecord.email == otp_in.email,
        OTPRecord.otp_code == otp_in.otp_code
    ).first()
